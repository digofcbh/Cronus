...
#define PVP_CALCRANK_INTERVAL 1000	// PVPè calculation interval
	...
	if( !changing_mapservers ) {
		...
		if (battle_config.display_version == 1){
			...
			if( strcmpi(get_git_revision(), "no") != 0 ) {
				sprintf(buf, "Revis„o GIT: '"CL_WHITE"%s"CL_RESET"'.\n", get_git_revision());
			} else if( strcmpi(get_svn_revision(), "no") != 0 ) {
				sprintf(buf, "Revis„o SVN: '"CL_WHITE"%s"CL_RESET"'.\n", get_svn_revision());
			} else {
				sprintf(buf, "Revis„o: Desconhecida");
			}
...
/*==========================================
 * êCalled when player changing job
...
* level_penalty.txt	- level specific values
*------------------------------------------*/
int pc_readdb(void)
{
	int i,j,k,tmp = 0;
