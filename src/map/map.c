...
int map_freeblock_unlock (void)
{
	if ((--block_free_lock) == 0) {
		...
		ShowError("map_freeblock_unlock: contagem de travados < 0 !\n");
		block_free_lock = 0;
	}
...
{
	if (block_free_lock > 0) {
		ShowError("map_freeblock_timer: block_free_lock(%d) é inválido.\n", block_free_lock);
		block_free_lock = 1;
...
int map_addblock(struct block_list* bl)
{
	...
	if( m < 0 || m >= map_num )
	{
		ShowError("map_addblock: id de mapa inválido (%d), apenas %d foram carregados.\n", m, map_num);
		return 1;
	}
	if( x < 0 || x >= map[m].xs || y < 0 || y >= map[m].ys )
	{
		ShowError("map_addblock: coordenadas fora da quadra (\"%s\",%d,%d), mapa é %dx%d\n", map[m].name, x, y, map[m].xs, map[m].ys);
		return 1;
	}
...
int map_foreachinrange(int (*func)(struct block_list*,va_list), struct block_list* center, int range, int type, ...)
{
	...
	if( bl_list_count >= BL_LIST_MAX )
		ShowWarning("map_foreachinrange: muitos blocos!\n");
...
int map_foreachinarea(int (*func)(struct block_list*,va_list), int m, int x0, int y0, int x1, int y1, int type, ...)
{
	...
	if( bl_list_count >= BL_LIST_MAX )
		ShowWarning("map_foreachinarea: muitos blocos!\n");
...
int map_forcountinrange(int (*func)(struct block_list*,va_list), struct block_list* center, int range, int count, int type, ...)
{
	...
	if( bl_list_count >= BL_LIST_MAX )
		ShowWarning("map_forcountinrange: muitos blocos!\n");
...
int map_foreachinmovearea(int (*func)(struct block_list*,va_list), struct block_list* center, int range, int dx, int dy, int type, ...)
{
	...
	if( bl_list_count >= BL_LIST_MAX )
		ShowWarning("map_foreachinmovearea: muitos blocos!\n");
...
int map_foreachincell(int (*func)(struct block_list*,va_list), int m, int x, int y, int type, ...)
{
	...
	if( bl_list_count >= BL_LIST_MAX )
		ShowWarning("map_foreachincell: muitos blocos!\n");
...
// kRO.
	...
	if( bl_list_count >= BL_LIST_MAX )
		ShowWarning("map_foreachinpath: muitos blocos!\n");
...
int map_foreachinmap(int (*func)(struct block_list*,va_list), int m, int type,...)
{
	...
	if( bl_list_count >= BL_LIST_MAX )
		ShowWarning("map_foreachinmap: muitos blocos!\n");
...
int map_get_new_object_id(void)
{
	...
	if( i == last_object_id ) {
		ShowError("map_addobject: id de objeto não livre!\n");
		return 0;
	}
...
int map_search_freecell(struct block_list *src, int m, short *x,short *y, int rx, int ry, int flag)
{
	...
	{
		ShowDebug("map_search_freecell: Uso incorreto! Quando src é NULL, flag tem que ser &1 e não pode ter &2\n");
		return 0;
	}
...
int map_removemobs_timer(int tid, unsigned int tick, int id, intptr_t data)
{
	...
	{	//Incorrect map id!
		ShowError("map_removemobs_timer error: timer %d aponta para mapa inválido %d\n",tid, m);
		return 0;
	}
	if (map[m].mob_delete_timer != tid)
	{	//Incorrect timer call!
		ShowError("map_removemobs_timer incompatibilidade: %d != %d (mapa %s)\n",map[m].mob_delete_timer, tid, map[m].name);
		return 0;
	}
	map[m].mob_delete_timer = INVALID_TIMER;
...
inline static struct mapcell map_gat2cell(int gat) {
	...
	switch( gat ) {
		...
		default:
			ShowWarning("map_gat2cell: tipo de gat não reconhecido '%d'\n", gat);
			break;
	}
...
static int map_cell2gat(struct mapcell cell)
{
	...
	ShowWarning("map_cell2gat: célula sem tipo de gat\n");
...
void map_setcell(int m, int x, int y, cell_t cell, bool flag)
{
	...
	switch( cell ) {
		...
		default:
			ShowWarning("map_setcell: tipo de célula inválido '%d'\n", (int)cell);
			break;
	}
}
...
int map_setipport(unsigned short mapindex, uint32 ip, uint16 port)
{
	...
	if(ip == clif_getip() && port == clif_getport()) {
		...
		ShowFatalError("map_setipport : recebida informação de que este map-server TEM QUE ter mapa '%s', mas não está carregado.\n",mapindex_id2name(mapindex));
		exit(EXIT_FAILURE);
	}
	mdos->ip   = ip;
	mdos->port = port;
...
static char *map_init_mapcache(FILE *fp)
{
	...
	if(fread(buffer, sizeof(char), size, fp) != size) {
		ShowError("map_init_mapcache: não foi possível ler todo o arquivo do mapcache\n");
		return NULL;
	}
...
int map_readfromcache(struct map_data *m, char *buffer, char *decode_buffer)
{
	...
	if( info && i < header->map_count ) {
		...
		if(size > MAX_MAP_SIZE) {
			ShowWarning("map_readfromcache: %s excedeu MAX_MAP_SIZE de %d\n", info->name, MAX_MAP_SIZE);
...
int map_addmap(char* mapname)
{
	...
	{
		ShowError("Não foi possível adicionar mapa '"CL_WHITE"%s"CL_RESET"', o limite de mapas foi atingido.\n",mapname);
		return 1;
	}
...
int map_readallmaps (void)
{
	...
	if( enable_grf )
		ShowStatus("Carregando mapas (usando arquivo GRF).\n");
	else {
		...
		if(!map_cache_buffer) {
			ShowFatalError("Falha em inicializar arquivo de mapcache (%s)..\n", mapcachefilepath);
			exit(EXIT_FAILURE);
		}
	}
	...
	for(i = 0; i < map_num; i++) {
		...
		{
			ShowWarning("Mapa %s já carregou!"CL_CLL"\n", map[i].name);
			if (map[i].cell) {
				aFree(map[i].cell);
				map[i].cell = NULL;
			}
...
int parse_console(const char* buf)
{
	...
	ShowNotice("Tipe de comando: '%s' || Comando: '%s' || Mapa: '%s' Coords: %d %d\n", type, command, map, x, y);

	if( n == 5 && strcmpi("admin",type) == 0 )
	{
		if( !is_atcommand(sd.fd, &sd, command, 0) )
			ShowInfo("Console: não é atcommand\n");
	}
	...
	{
		ShowInfo("Para usar comandos de GM:\n");
		ShowInfo("  admin:<gm command>:<map of \"gm\"> <x> <y>\n");
		ShowInfo("Você pode usar qualquer comando de GM que não requeira GM.\n");
		ShowInfo("Você pode usar o @charwarp\n");
		ShowInfo("O <mapa de \"gm\"> <x> <y> é para comandos que requerem coords. do GM.\n");
		ShowInfo("Ex.: @spawn\n");
...
int map_config_read(char *cfgName)
{
	...
	if( fp == NULL )
	{
		ShowError("Arquivo de configuração de mapas não encontrado em: %s\n", cfgName);
		return 1;
	}
		...
		*ptr = '\0';
		...
		if(strcmpi(w1,"timestamp_format")==0)
			...
			if( msg_silent ) // only bother if its actually enabled
				ShowInfo("Silenciosidade do console: %d\n", atoi(w2));
			...
			ShowWarning("Configuração '%s' desconhecido no arquivo %s\n", w1, cfgName);
	}
...
void do_abort(void)
{
	...
	if (run) {
		ShowFatalError("Servidor travou enquanto salvava os chars. Infos. dos chars não podem ser salvos!\n");
		return;
	}
	run = 1;
	...
	{
		if (pc_db->size(pc_db))
			ShowFatalError("Servidor travou sem conexão ao char-server, %u chars não podem ser salvos!\n", pc_db->size(pc_db));
		return;
	}
	ShowError("Servidor recebeu alerta de travamento! Tentando salvar todos os chars online!\n");
...
static bool map_arg_next_value(const char* option, int i, int argc)
{
	if( i >= argc-1 )
	{
		ShowWarning("Valor para opção '%s' perdido.\n", option);
		return false;
	}
