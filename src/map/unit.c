	...
	if (x<0 || y<0)
  	{	//Random map position.
		if (!map_search_freecell(NULL, m, &x, &y, -1, -1, 1)) {
			ShowWarning("unit_warp falhou. Id da unidade:%d/Tipo:%d, posi鈬o do alvo no mapa %d (%s) em [%d,%d]\n", bl->id, bl->type, m, map[m].name, x, y);
			return 2;

		}
	} else if (map_getcell(m,x,y,CELL_CHKNOREACH))
	{	//Invalid target cell
		ShowWarning("unit_warp: C駘ula n縊 v疝ida para movimenta鈬o: %d (%s) at [%d,%d]\n", m, map[m].name, x,y);
	...
	switch( bl->type ) {
		case BL_PC: {
			...
			{// this is only place where map users is decreased, if the mobs were removed too soon then this function was executed too many times [FlavioJS]
				...
				ShowDebug("unit_remove_map: estado inesperado durante a remo鈬o de jogador AID/CID:%d/%d"
					" (ativo=%d connect_new=%d rewarp=%d changemap=%d debug_remove_map=%d)"
					" do mapa=%s (jogadores=%d)."
					" ﾚltima chamada de %s:%d(%s), chamada atual de %s:%d(%s)."
