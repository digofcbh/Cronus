...
void chrif_checkdefaultlogin(void) {
	if( strcmp(userid, "s1") == 0 && strcmp(passwd, "p1") == 0 ) {
		...
		ShowNotice("\ttipo S (servidor) que tenha login e senha diferente do padrão.\n");
		ShowNotice("Lembre-se de editar os arquivos de configuração (*.conf) para estes usarem\n");
		ShowNotice("a nova conta com login e senha não-padronizados.\n");
	}
}
...
int chrif_setip(const char* ip) {
	...
	ShowInfo("Endereço IP do char-server: '"CL_WHITE"%s"CL_RESET"' -> '"CL_WHITE"%s"CL_RESET"'.\n", ip, ip2str(char_ip, ip_str));
	return 1;
}
...
int chrif_connect(int fd) {
	ShowStatus("Fazendo login no char-server...\n", char_fd);
	WFIFOHEAD(fd,60);
	WFIFOW(fd,0) = 0x2af8;
...
int chrif_removemap(int fd) {
	...
	other_mapserver_count--;
	if(battle_config.etc_log)
		ShowStatus("Removendo mapa do server %d.%d.%d.%d:%d (%d mapas)\n", CONVIP(ip), port, j);
	return 0;
}
...
int chrif_changemapserverack(int account_id, int login_id1, int login_id2, int char_id, short map_index, short x, short y, uint32 ip, uint16 port) {
	...
	if (!login_id1) {
		ShowError("Mudança do map-server falhou.\n");
...
int chrif_connectack(int fd) {
	...
	if (RFIFOB(fd,2)) {
		ShowFatalError("Conexão ao char-server falhou %d.\n", RFIFOB(fd,2));
		exit(EXIT_FAILURE);
	}
...
int chrif_sendmapack(int fd) {
	if (RFIFOB(fd,2)) {
		ShowFatalError("chrif : falha em enviar lista de mapas ao char-server %d\n", RFIFOB(fd,2));
		exit(EXIT_FAILURE);
	}
...
int chrif_changesex(struct map_session_data *sd) {
	...
	clif_displaymessage(sd->fd, msg_txt(408));
...
int chrif_changedsex(int fd) {
	...
	if (sd) { //Normally there should not be a char logged on right now!
		...
		clif_displaymessage(sd->fd, msg_txt(409));
