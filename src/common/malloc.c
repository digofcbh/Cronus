 ...
 *       ?j?b?g???A1024?W????do???A??????A64Byte?~??j?b?g?? 512?W?????
...
struct block {
	...
	unsigned short unit_count;		/* ???j?b?g??? */
...
static void memmgr_log ( char *buf ) {
	if( !log_fp ) {
		...
		time( &raw );
		t = localtime( &raw );
		fprintf( log_fp, "\nMemory manager: Memory leaks found at %d/%02d/%02d %02dh%02dm%02ds (GIT: %s/SVN: %s).\n",
				 ( t->tm_year + 1900 ), ( t->tm_mon + 1 ), t->tm_mday, t->tm_hour, t->tm_min, t->tm_sec, get_git_revision(), get_svn_revision() );
	}
